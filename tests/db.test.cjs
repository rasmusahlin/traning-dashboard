...
test('history pagination respects a lower server cap and keeps requesting until empty', async () => {
  ...
  let requests = 0;
  const context = vm.createContext({ URLSearchParams, console, Date, JSON, Promise,
    localStorage: { getItem: () => JSON.stringify({ access_token: 'synthetic', expires_at: Date.now()+3600000 }), removeItem() {} },
    fetch: async url => {
...
test('reauthentication reloads page state before exposing a different account', async () => {
  ...
  const context=vm.createContext({console,Date,JSON,Promise,URLSearchParams,
    localStorage:{getItem:()=>JSON.stringify({access_token:'old',expires_at:Date.now()+3600000,user:{id:'old-owner'}}),setItem(){},removeItem(){}},
    document:{getElementById:()=>({})}, location:{reload(){reloaded++;}},
    fetch:async()=>({ok:true,json:async()=>({access_token:'new',expires_in:3600,user:{id:'new-owner'}})})});
