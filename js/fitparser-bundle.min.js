...
function readData(blob, fDef, startIndex, options) {
    ...
    if (fDef.type === 'string') {
        ...
        return String.fromCharCode.apply(null, new Uint8Array(_temp));
    }
        ...
        for (var _i5 = 0; _i5 < numberOfDeveloperDataFields; _i5++) {
            ...
            try {
                ...
                var _fDef = {
                    ...
                    dataType: getFitMessageBaseType(_baseType & 15),
...
// === helper.js ===
function _toConsumableArray(arr) { return Array.from(arr); }
var mapDataIntoLap = function(inputLaps, lapKey, data) {
  var laps = _toConsumableArray(inputLaps), index = 0;
  for (var i = 0; i < laps.length; i++) {
    var lap = laps[i], nextLap = laps[i+1], tempData = [];
    var lapStart = new Date(lap.start_time).getTime();
    var nextLapStart = nextLap ? new Date(nextLap.start_time).getTime() : null;
    for (var j = index; j < data.length; j++) {
      var row = data[j];
      if (nextLap) {
        var ts = new Date(row.timestamp).getTime();
        if (lapStart <= ts && nextLapStart > ts) { tempData.push(row); }
        else if (nextLapStart <= ts) { index = j; break; }
      } else { tempData.push(row); }
    }
    if (!laps[i][lapKey]) laps[i][lapKey] = tempData;
  }
  return laps;
};
var mapDataIntoSession = function(inputSessions, laps) {
  var sessions = _toConsumableArray(inputSessions), lapIndex = 0;
  for (var i = 0; i < sessions.length; i++) {
    var session = sessions[i], nextSession = sessions[i+1], tempLaps = [];
    var sessStart = new Date(session.start_time).getTime();
    var nextSessStart = nextSession ? new Date(nextSession.start_time).getTime() : null;
    for (var j = lapIndex; j < laps.length; j++) {
      var lap = laps[j];
      if (nextSession) {
        var ls = new Date(lap.start_time).getTime();
        if (sessStart <= ls && nextSessStart > ls) { tempLaps.push(lap); }
        else if (nextSessStart <= ls) { lapIndex = j; break; }
      } else { tempLaps.push(lap); }
    }
    if (!sessions[i].laps) sessions[i].laps = tempLaps;
  }
  return sessions;
};
  ...
  _createClass(FitParser, [{
    ...
    value: function parse(content, callback) {
      ...
      while (loopIndex < crcStart) {
        var _readRecord = readRecord(blob, messageTypes, developerFields, loopIndex, this.options, startDate, pausedTime),
